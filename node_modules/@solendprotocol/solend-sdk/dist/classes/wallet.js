...
class SolendWallet {
    ...
    static initialize(wallet, connection, environment = "production") {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const config = (yield (yield axios_1.default.get(`${API_ENDPOINT}/v1/config?deployment=${environment}`)).data);
            market.config = config;
            yield market.loadRewards();
            return market;
        });
    }
    loadRewards() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const lmStatResponse = (yield axios_1.default.get(`${API_ENDPOINT}/liquidity-mining/reward-stats-v2`)).data;
            const lmScoreResponse = (yield axios_1.default.get(`${API_ENDPOINT}/liquidity-mining/reward-score-v2?wallet=${this.provider.wallet.publicKey.toBase58()}`)).data;
            ...
            const primaryMarketSeed = this.config.markets
                ...
                .address.slice(0, 32);
            const obligationAddress = yield web3_js_1.PublicKey.createWithSeed(this.provider.wallet.publicKey, primaryMarketSeed, new web3_js_1.PublicKey(this.config.programID));
            ...
            const [lmStatData, lmScoreData, externalStatData, externalScoreData, claimData,] = yield Promise.all([
                lmStatResponse,
                lmScoreResponse,
            ...
            const slndEarningsData = Object.entries(lmScoreData).reduce((acc, [poolKey, pool]) => {
                const sum = pool.reduce((acc, rewardScore) => {
                    if (!lmStatData[poolKey] ||
                        !lmStatData[poolKey][rewardScore.tokenMint] ||
                        !lmStatData[poolKey][rewardScore.tokenMint][rewardScore.side])
                        return acc;
                    const rewardStat = lmStatData[poolKey][rewardScore.tokenMint][rewardScore.side];
                    const currentScore = rewardStat
                        ? (0, utils_1.estimateCurrentScore)(rewardStat, rewardScore, mostRecentSlot, mostRecentSlotTime).toNumber()
                        : 0;
            ...
            const rewardsData = Object.assign(Object.assign({}, externalEarningsData), { SLNDpmoWTVADgEdndyvWzroNL7zSi1dF9PC3xHGtPwp: {
                    symbol: "SLND",
